...
In order to obtain the correct input for the AnophelsModel package, the function
Bionomics_For_Anophles_Model() computes species-specific bionomic parameters using a hierarchical Bayesian framework. It wraps and extends the logic of species_complex_result(), applying it sequentially to the core bionomic traits estimated by the model: parous rate (M, M.sd), endophagy (endophagy, endophagy.sd), endophily (endophily, endophily.sd), sac rate (A0, A0.sd), indoor and outdoor human blood indices, and resting duration.
...
The output format is specifically designed to plug directly into AnophelesModel.
